...
def main(args=None):
    ...
    parser.add_argument(
        '--wrca', action='store_true', help='use --wrca if you are processing a WRCA file. These are processed differently because the filenames and identifiers are nonstandard.')
    parser.add_argument(
...
def process(eadFile, isWRCA, keepRaw):
    ...
    #write out to file
    writeOut(newXML, eadFile, isWRCA, eadID, keepRaw)
...
def writeOut(newXML, eadPath, isWRCA, eadID, keepRaw):
