...
def process(ead_file):
    """Return EAD edited according to UCR SCUA standard practice, along with its <eadid>.
    ...
    for langname in isodict:
        try:
            if langname in langmat.text:
                langmarkup = '<language langcode="' + isodict.get(langname) + r'"\>' +  langname + '</language>'
    ...
    new_xml = re.sub(r'xmlns:xs="http:\/\/www\.w3\.org\/2001\/XMLSchema"\s+xmlns:ead="urn:isbn:1-931666-22-9"', '', new_xml)
...
def iso639():
    ...
    #This was originally split out into a separate file and imported, but put back here because of running into errors upon packaging.
    #TODO: this is unwieldy. either split back into its own file or find an external library that can do this
